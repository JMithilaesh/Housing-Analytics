...
def drop_columns(df):
    df = df[['AreaSpace_SQFT', 'Bedrooms', 'Bathrooms', 'Parking', 'ZipCode', 'Price']]
    return df
...
def drop_zero(df):
    df = df[(df[['AreaSpace_SQFT', 'Bedrooms', 'Bathrooms', 'ZipCode', 'Price']] != 0).all(axis=1)]
    return df
...
# Regions from: https://gafccla.com/region-map.php

Region1 = ['Bartow', 'Catoosa', 'Chattooga', 'Cobb', 'Dade', 'Floyd', 'Gordon', 'Murray', 'Paulding', 'Polk', 'Walker', 'Whitfield']
Region2 = ['Barrow', 'Cherokee', 'Dawson', 'Fannin', 'Forsyth', 'Gilmer', 'Gwinnett', 'Hall', 'Lumpkin', 'Pickens', 'Union', 'Walton']
Region3 = ['Banks', 'Clarke', 'Elbert', 'Franklin', 'Habersham', 'Hart', 'Jackson', 'Madison', 'Oconee', 'Oglethorpe', 'Rabun', 'Stephens', 'Towns', 'White']
Region4 = ['Carroll', 'Clayton', 'Coweta', 'Douglas', 'Fayette', 'Fulton', 'Haralson', 'Harris', 'Heard', 'Meriwether', 'Pike', 'Spalding', 'Talbot', 'Troup', 'Upson']
Region5 = ['Butts', 'Dekalb', 'Henry', 'Lamar', 'Monroe', 'Rockdale']
Region6 = ['Baldwin', 'Burke', 'Columbia', 'Glascock', 'Greene', 'Hancock', 'Jasper', 'Jefferson', 'Johnson', 'Jones', 'Lincoln', 'McDuffie', 'Morgan', 'Newton', 'Putnam', 'Richmond', 'Talifero', 'Twiggs', 'Warren', 'Washington', 'Wilkes', 'Wilkinson']
Region7 = ['Baker', 'Bibb', 'Calhoun', 'Chattahoochee', 'Clay', 'Crawford', 'Crisp', 'Decatur', 'Dooly', 'Dougherty', 'Early', 'Grady', 'Houston', 'Lee', 'Macon', 'Marion', 'Mitchell', 'Muscogee', 'Peach', 'Quitman', 'Randolph', 'Schley', 'Seminole', 'Stewart', 'Sumter', 'Taylor', 'Terrell', 'Webster', 'Worth']
Region8 = ['Atkinson', 'Ben', 'Hill', 'Berrien', 'Bleckley', 'Brooks', 'Clinch', 'Coffee', 'Colquitt', 'Cook', 'Dodge', 'Echols', 'Irwin', 'Jeff Davis', 'Lanier', 'Laurens', 'Lowndes', 'Montgomery', 'Pulaski', 'Telfair', 'Thomas', 'Tift', 'Treutlen', 'Turner', 'Ware', 'Wheeler', 'Wilcox']
Region9 = ['Appling', 'Bacon', 'Brantley', 'Bryan', 'Bulloch', 'Camden', 'Candler', 'Charlton', 'Chatham', 'Effingham', 'Emanuel', 'Evans', 'Glynn', 'Jenkins', 'Liberty', 'Long', 'McIntosh', 'Pierce', 'Screven', 'Tattnall', 'Toombs', 'Wayne']

Region = [Region1, Region2, Region3, Region4, Region5, Region6, Region7, Region8, Region9]

for i in Region:
    print('For ' + str(i) + '#################################################################')
    R1_Zip = db[colZip].distinct("zip", {'state_id': 'GA', "county_name": {"$in": i}})
    R1_Zip = [str(int(i)).zfill(5) for i in R1_Zip]
    # print(R1_Zip)

    myquery = {"ZipCode": {"$in": R1_Zip}, "Status": {"$regex": "sale"}}
    ...
    plt.scatter(df2['AreaSpace_SQFT'], df2['Price'])

    km = KMeans(n_clusters=3)
    # k_pred=km.fit_predict(df2[['Price','AreaSpace_SQFT']])
    ...
    df2 = df2[['AreaSpace_SQFT', 'Bedrooms', 'Bathrooms', 'ZipCode', 'cluster', 'Price']]
    # print(df2.head())
    df3 = df2.values
    ...
    myquery3 = {"ZipCode": {"$in": R1_Zip}, "Status": {"$regex": "rent"}}
    mydoc = db[col].find(myquery3)

    for x in mydoc:
        if x['AreaSpace_SQFT'] is None or isNaN(x['AreaSpace_SQFT']) or x['AreaSpace_SQFT'] == "":
            x['AreaSpace_SQFT'] = 0
        if x['Bedrooms'] is None or isNaN(x['Bedrooms']) or x['Bedrooms'] == "":
            x['Bedrooms'] = 0
        ...
        # print(x['_id'])
        print("p: ", p)
        db[col].update_one({"_id": x["_id"]}, {"$set": {"PredictionPrice": p}})